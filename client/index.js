...
const getClientDbConfigs = async () => {
  ...
  try {
    ...
    const [agencies] = await connection.query(
      "SELECT agence_ip, agence_nom FROM tb_agence WHERE agence_ip != '10.56.11.28' ORDER BY agence_ip;" // Récupérer les adresses des bases de données des agences
    );
      ...
      try {
        ...
        if (typeof contrainte === "string" && contrainte.trim().length > 0) {
          ...
          const [users] = await connection.query(
            `SELECT u.* FROM tb_ticket t JOIN tb_users u ON t.ticket_user_login = u.user_login WHERE t.ticket_date2 BETWEEN DATE_SUB(CURDATE(), INTERVAL 1 WEEK) AND CURDATE() GROUP BY u.user_login ORDER BY u.user_login ASC;`
          );
          ...
          const [role_user] = await connection.query(`
            ...
            JOIN tb_role r ON ru.role_id = r.role_id 
			      JOIN tb_ticket t ON t.ticket_user_id = u.user_id  WHERE t.ticket_date2 BETWEEN DATE_SUB(CURDATE(), INTERVAL 1 WEEK) AND CURDATE()
