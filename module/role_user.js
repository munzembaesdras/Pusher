  ...
  try {
    for (const record of records) {
      ...
      if (userRows.length === 0) {
        //console.log(  `Aucun utilisateur trouvé avec le login "${user_login}". L'attribution de rôle est annulée.`);
        continue;
      }
      ...
      if (existingRoleUserRows.length > 0) {
        if (existingRoleUserRows.length > 1) {
          ...
          existingRoleUserRows.forEach(async (row) => {
            const supression = await connection.execute(
              "DELETE FROM tb_role_user WHERE role_user_id = ?",
              [row.role_user_id]
            );
